...
""")) 
    ...
    def loyalty_club(self):
        ...
        if result:
            cursor.execute(f"SELECT totalprice FROM customers WHERE user_name='{self.user_name}'")
            ...
            if totalprice>=100000:
                ...
                cursor.execute(f"UPDATE customers SET discount = '{self.discount}' WHERE user_name = '{username}' AND password='{password}'") 
                connect.commit() 
            elif totalprice>=500000:
                cursor.execute(f"SELECT wallet FROM customers WHERE user_name='{self.user_name}'")
                wallet=cursor.fetchone()[0]
                self.wallet=wallet
                connect.commit()
                discount+=50000
                cursor.execute(f"UPDATE customers SET wallet = '{self.wallet}' WHERE user_name = '{username}' AND password='{password}'")
                connect.commit()
